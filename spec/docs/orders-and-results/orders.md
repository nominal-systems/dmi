...
Some providers may require additional requisition parameters for specific services and tests. These parameters are described in the `labRequisitionInfo` property of the [Service](https://nominal.stoplight.io/docs/dmi/schemas/service) model, obtained when [querying the directory of services](https://nominal.stoplight.io/docs/dmi/api/operations/list-provider-services) for a provider. 

These parameters, if required, must be sent in the `labRequisitionInfo` property of the [create order](https://nominal.stoplight.io/docs/dmi/api/operations/create-a-order) endpoint body. 
